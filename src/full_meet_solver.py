...
class FullMeetSolver:
    ...
    def __init__(self, swimmers: list[Swimmer], num_days: int) -> None:
    ...
    def __repr__(self) -> str:
        ...
        return f"FullMeetSolver(num_females={self.num_females}, num_males={self.num_males})"
    ...
    def _get_data(self) -> None:
        ...
        for day in range(1, self.num_days + 1):
            ...
            self.female_points[day - 1] = [x.projected_points[day - 1] for x in self.female_swimmers]
            # male projected points from greatest to least
            self.male_points[day - 1] = [x.projected_points[day - 1] for x in self.male_swimmers]
    ...
    def _get_solution(self) -> None:
        ...
        for day in range(self.num_days):
            for index, swimmer in enumerate(self.female_swimmers):
                objective_terms.append(swimmer.projected_points[day] * (female_vars[day][index] + female_captain_vars[day][index]))
            for index, swimmer in enumerate(self.male_swimmers):
                objective_terms.append(swimmer.projected_points[day] * (male_vars[day][index] + male_captain_vars[day][index]))
        ...
        for day in range(self.num_days):
            budget_terms = []
            for index, swimmer in enumerate(self.female_swimmers):
                budget_terms.append(swimmer.cost * female_vars[day][index])
            for index, swimmer in enumerate(self.male_swimmers):
                budget_terms.append(swimmer.cost * male_vars[day][index])

            self.solver.Add(sum(budget_terms) <= BUDGET)

        # Number of females constraints
        for day in range(self.num_days):
            self.solver.Add(sum(female_vars[day]) == ROSTER_SIZE // 2)

        # Number of males constraints
        for day in range(self.num_days):
            self.solver.Add(sum(male_vars[day]) == ROSTER_SIZE // 2)

        # Captain constraints (can't be captain if not in lineup)
        for day in range(self.num_days):
            for swimmer_var, captain_var in zip(female_vars[day] + male_vars[day], female_captain_vars[day] + male_captain_vars[day], strict=True):
                self.solver.Add(captain_var <= swimmer_var)
            self.solver.Add(sum(male_captain_vars[day] + female_captain_vars[day]) == 1)

        # Total switches constraint (switches are double counted)
        self.solver.Add(sum(day_switch_counts) / 2 <= SWITCHES)

        # Day switch count constraints
        for day in range(1, self.num_days):
            self.solver.Add(day_switch_counts[day - 1] == sum(switch_vars[day - 1]))

        # Switch constraints
        for day in range(1, self.num_days):
            for index in range(self.num_females):
                x = female_vars[day][index]
                y = female_vars[day - 1][index]
                z = switch_vars[day - 1][index]
                self.solver.Add(z >= x - y)
                self.solver.Add(z >= y - x)
                self.solver.Add(z <= x + y)
                self.solver.Add(z <= 2 - (x + y))
            for index in range(self.num_males):
                x = male_vars[day][index]
                y = male_vars[day - 1][index]
                z = switch_vars[day - 1][index + self.num_females]
                self.solver.Add(z >= x - y)
                self.solver.Add(z >= y - x)
                self.solver.Add(z <= x + y)
                self.solver.Add(z <= 2 - (x + y))
    ...
    def _print_solution(self) -> None:
        ...
        for day, day_solution_values in enumerate(self.solution_values["swimmer_decision_vars"]):
            ...
            for swimmer in female_swimmers:
                if self.solution_values["captain_decision_vars"][day][self.female_swimmers.index(swimmer)]:
                    print(f"{swimmer.name} ({swimmer.projected_points[day] * 2}) (Captain)")
                    total_points += swimmer.projected_points[day] * 2
                else:
                    print(f"{swimmer.name} ({swimmer.projected_points[day]})")
                    total_points += swimmer.projected_points[day]
            for swimmer in male_swimmers:
                if self.solution_values["captain_decision_vars"][day][self.male_swimmers.index(swimmer) + self.num_females]:
                    print(f"{swimmer.name} ({swimmer.projected_points[day] * 2}) (Captain)")
                    total_points += swimmer.projected_points[day] * 2
                else:
                    print(f"{swimmer.name} ({swimmer.projected_points[day]})")
